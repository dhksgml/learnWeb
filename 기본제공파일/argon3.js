...
//원-사각형 충돌처리
function jwh14_distance(x0,y0,x1,y1) {
  return (x0 - x1) * (x0 - x1) + (y0 - y1) * (y0 - y1);
}

function jwh14_hitCircleToRect(tc, pc) {

  var jwh14_nResult = false;
  if(pc.x + pc.w/2> tc.x && pc.x - pc.w/2 < tc.x + tc.w && pc.y + pc.w/2 > tc.y && pc.y - pc.w/2 < tc.y + tc.h){
      jwh14_nResult = true;
      if(pc.x < tc.x){
          if(pc.y < tc.y){
              if(jwh14_distance(tc.x, tc.y, pc.x, pc.y) >= (pc.w/2) * (pc.w/2)){
                jwh14_nResult = false;
              }
          }
          else{
              if(pc.y > tc.y + tc.h){
                  if(jwh14_distance(tc.x, tc.y + tc.h, pc.x, pc.y) >= (pc.w/2) * (pc.w/2)){
                    jwh14_nResult = false;
                  }
              }
          }
          ...
          if(pc.x > tc.x + tc.w){
              if(pc.y < tc.y){
                  if(jwh14_distance(tc.x + tc.w, tc.y, pc.x, pc.y) >= (pc.w/2) * (pc.w/2)){
                    jwh14_nResult = false;
                  }
              }
              else{
                  if(pc.y > tc.y + tc.h){
                      if(jwh14_distance(tc.x + tc.w, tc.y + tc.h, pc.x, pc.y) >= (pc.w/2) * (pc.w/2)){
                        jwh14_nResult = false;
                      }
                  }
              }
...
function jwh14_collisionShip() {
  ...
  if(stype > 2){                    
      ...
      for(i = 0; i < arrEnemy.length; i++){               
        if(jwh14_hitCircleToRect(jwh14_bboxship2,arrEnemy[i])){   //확장된 콜라이더와 적이 충돌 시
  ...
  for(i = 0; i < arrEnemy.length; i++){   
      if(jwh14_hitCircleToRect(jwh14_bboxship, arrEnemy[i])){   //기본 콜라이더와 적 충돌 시      
          jwh14_createParticle(arrEnemy[i])             //파티클 생성
...
function jwh14_collisionRocket(nowR, pos) {           //로켓 충돌 검사 함수
  var i;
  for(i = 0; i < arrEnemy.length; i++){
      if(jwh14_hitCircleToRect(nowR,arrEnemy[i])){          //적과 로켓이 충돌했을 경우
...
function jwh14_drawParticle(tp) {             
  ...
  for(i=0; i<tp.length; i+=1){
      ...
      ctx.beginPath();
      ctx.arc(tp[i].px, tp[i].py, tp[i].pw/2,0,Math.PI*2);
      ctx.fill();
...
function createEnemy() {
  var jwh14_tx, jwh14_ty, jwh14_twh, jwh14_tc, jwh14_tv, jwh14_r;

  jwh14_tx = Math.floor(Math.random() * vcanvas.width);
  jwh14_twh = Math.floor(Math.random() * 31) + 10;
  jwh14_r = Math.floor(Math.random() * 26) + 6; //  반지름
  jwh14_ty = 0
  jwh14_tc = Math.round(360*Math.random());
  jwh14_tv = Math.floor(Math.random() * 2) + 1;
  if(jwh14_tx - jwh14_twh > 0 && jwh14_tx + jwh14_twh < vcanvas.width){ //안 잘리게 보정
    arrEnemy.push({ x: jwh14_tx, y: jwh14_ty, w: jwh14_twh, h: jwh14_twh, c: jwh14_tc, v: jwh14_tv, hit:0 ,r: jwh14_r});
  }
}
